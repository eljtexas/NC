#purpose: take a VJ/intron fasta file of concatenated variable region and intron and align it, such that the coding region is aligned as AA and the
...
#usage: python VJintron_alignment.py fasta_file 
...
def cdr3_frame(antibody):
	...
	AA_seq=str(antibody.translate())[-40:] #force the motif search to be at the end of the light chain (last 40 AAs)

	
	match=re.search(r"C[A-Z]{5,20}F", AA_seq) #utilizing light chain motif definitions 
	...
	if match!=None:
		...
		if len(antibody)%3!=0:
			...
			while len(antibody)%3!=0:
				antibody=antibody[1:] #after removing trailing, if there's still a problem, remove from the beginning again (insertion in primer site)
	...
	if match==None:
		antibody=antibody[1:]
		antibody=Seq(str(antibody))
		AA_seq=str(antibody.translate())[-40:] #make sure only looking at the last 40
		match=re.search(r"C[A-Z]{5,20}F", AA_seq)
		if match!=None:
			CDR3=(match.group())[:-3]
			if len(antibody)%3!=0:
				antibody=antibody[:-1] #remove any trailing nucleotide
			while len(antibody)%3!=0:
				antibody=antibody[1:] #again check and remove from 5' if necessary
