#purpose: take a stitched and quality filtered fasta file and use IMGT info to keep only
#the productive reads with three CDRs, 4 FWRs and annotated to IGLV1-51 and IGLJ1 genes
#usage: python productive_CDRs_FWRs_IGLV1-51_IGLJ1.py 1_IMGT_summary_file
...
def filter(dictionary, outputfile):
	...
	for key in dictionary:
		...
		if functionality=='productive':
			if v_gene.find("IGLV1-51")!=-1:
				if j_gene.find("IGLJ1")!=-1:
					if cdrs.find("X")==-1:
						if frs.find("X")==-1:
							outputfile.write(">{0}\n".format(key))
