...
#usage: python V_gene_intron_divergence_plots_mafft_non_multiplex_IGLJ1.py fasta_file 1_IMGT_file 3_IMGT_file 8_IMGT_file timepoint_name
...
#	print num_point_mutations		
	...
	total_nt=193 #to get mutation rate divide by the length of the read to help normalize this rate? #maybe this normalization is just ruining my rates? what am i trying to see, lets leave it at 109				
...
#	temp_output_pairwise_alignment=open("temp_output_pairwise_mafft.fasta",'w')
	#temp_pair="temp_output_pairwise_mafft.fasta"

#	for line in germline_file:
#		stripline=line.rstrip()
#		temp_output_pairwise_alignment.write("{0}\n".format(stripline))
	#now the germline sequence was added to the temp file
	#now add in one intron, align, parse, record, delete and repeat the process
	
	for key in IMGT_summary_database:

		germline_file=open("human_jl101_intron.fasta")
			...
			#temp_output_pairwise_alignment.seek(0)
...
def divergence_plot(sequence_database, IMGT_summary_database):
	...
	#binwidth=1
	#x=np.arange(min(intron_data), max(intron_data)+binwidth,binwidth)
	#y=np.arange(min(v_gene_data), max(v_gene_data)+binwidth,binwidth)
	...
	#cbar.set_ticks([0.055, 0.075, 0.095])

#	plt.hist2d(intron_data,v_gene_data, normed=True) #default bins=10
#	cb=plt.colorbar()
#	plt.title('Week {0}'.format(sys.argv[5]), fontsize=30)
 #       plt.xlabel('Intron Divergence (Muts+Indel Events) (%)', fontsize=30)
  #      plt.ylabel('V Gene Divergence (%)', fontsize=30)
#	plt.gcf().subplots_adjust(bottom=0.15)
#	plt.axis([0,25,0,60])
        #plt.savefig("{0}_Vgene_intron_scatter.png".format(sys.argv[1]))
	x=np.arange(0,30,0.001) #arange makes arrays
